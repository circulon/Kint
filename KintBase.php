...
class KintBase
{
	 ...
	 *      self::MODE_* - enable and force selected mode disregarding detection and function
	...
	/**
	 * Prints a debug backtrace, same as self::dump(1)
	 *
	 ...
	 *   ~ self::dump()
	 *
	 * Modifiers are supported by all dump wrapper functions, including self::trace(). Space is optional.
	 *
	 *
	 * You can also use the following shorthand to display debug_backtrace():
	 *   self::dump( 1 );
	 *
	 * Passing the result from debug_backtrace() to self::dump() as a single parameter will display it as trace too:
	...
	public static function dump( $data = null )
	{
		...
		$trace = false;
		if ( $names === array( null ) && func_num_args() === 1 && $data === 1 ) { # self::dump(1) shorthand
